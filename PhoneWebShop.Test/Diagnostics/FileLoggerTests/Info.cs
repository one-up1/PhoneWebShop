...
namespace PhoneWebShop.Business.Test.Diagnostics.FileLoggerTests
{
    ...
    public class Info
    {
        ...
        public void Log_Should_HaveCorrectFormat()
        {
            ...
            // Assert
            var fileContent = File.ReadAllText(_mockOptions.Object.Value.FileLoggerOutputLocation);
            fileContent.Should().Match("*/*/???? *:*:* ?? (I) INFO - *");
        ...
        [Fact]
        public void Should_PutTimeInFrontOfMessage()
        {
            RemoveTestFile();

            // Arrange
            var expectedTime = DateTime.Now;
            _mockTimeProvider.Setup(provider => provider.NowTime).Returns(expectedTime);

            // Act
            _classToBeTested.Info("Message");

            // Assert
            var fileContent = File.ReadAllText(_mockOptions.Object.Value.FileLoggerOutputLocation).Replace("\r\n", string.Empty);
            fileContent.Should().Be($"{expectedTime} (I) INFO - Message");
